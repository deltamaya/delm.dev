<script>
    ...
    let mobileLayout = $state(true);
    let {dark} = $props();

    import {fly} from 'svelte/transition'

    const mobileLayoutThreshold = 600;
    ...
    >
        <div class="flex lg:w-[1024px] md:w-[720px] sm:w-[500px] ">
            <div class="flex flex-wrap font-extrabold lg:gap-10 md:gap-7 gap-5">
                <img src={dark?"logo-white.png":"logo.png"}
                     class="lg:w-[70px] lg:h-[40px] md:w-[60px] md:h-[35px] w-[50px] h-[28px]" alt="logo"/>
                <HeaderItem title="Projects" id="projects"/>
                <HeaderItem title="Info" id="info"/>
                <HeaderItem title="Links" id="links"/>
            </div>
        </div>
    </div>
{:else }
